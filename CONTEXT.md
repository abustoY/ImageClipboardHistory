# プロジェクトコンテキスト: ImageClipboardHistory (Piccy)
...
### B. レイアウト構造 (バーのコンパクト化)
- **ヘッダー**: 左側に「Image History」タイトル、右端に「ゴミ箱」ボタンを配置。縦幅は最小限に抑える。
...
- **Gitコミットの禁止**: **ユーザーの明示的な許可なく `git commit` コマンドを実行しないでください。** 実装が完了しても、必ずユーザーに確認を取り、指示を受けてからコミットを行うように徹底してください。
